...
export default function ShowroomView({ user, activeBattle, entries, myCars, myEntryData }: any) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground transition-colors duration-500 pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
      
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-[-10%] right-[-10%] w-[70vw] h-[70vw] bg-primary/10 rounded-full blur-[120px]" />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-8">
        ...
        <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="bg-primary/10 border border-primary/20 px-6 py-3 rounded-2xl flex items-center gap-3">
             ...
             <span className="text-xs font-black uppercase text-primary tracking-widest">{playableEntries.length} Új setup vár</span>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* JOBB OLDAL (7 oszlop) - A Játék Színpada */}
          <div className="lg:col-span-7 flex flex-col items-center">
            <div className="w-full glass rounded-[3rem] p-8 border-neon-glow min-h-[600px] flex flex-col items-center justify-center relative overflow-hidden">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 mt-8 flex items-center gap-2 bg-accent/50 p-1.5 px-4 rounded-full border border-border/50 z-10">
                ...
                <div className="w-full max-w-[380px] z-10">
                  {playableEntries.length > 0 ? (
                    ...
                    <div className="text-center py-12">
                      <Trophy size={64} className="mx-auto mb-6 text-primary opacity-30" />
                      <h3 className="text-2xl font-black uppercase italic tracking-tighter text-foreground mb-2">Vége a körnek!</h3>
