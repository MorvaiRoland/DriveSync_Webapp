...
export default function VinCheckPage() {
  ...
  const DataItem = ({ icon, label, value, sub }: any) => (
      <div className="flex items-start gap-3 p-3 md:p-4 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors group">
          <div className="p-2 md:p-3 rounded-xl bg-slate-900 text-slate-400 group-hover:text-emerald-400 transition-colors shadow-inner flex-shrink-0">
              {icon}
          </div>
          <div className="min-w-0">
              <div className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-0.5 md:mb-1 truncate">{label}</div>
              <div className="text-sm md:text-base font-bold text-white leading-tight truncate">{value || '-'}</div>
              {sub && <div className="text-[10px] md:text-xs text-slate-400 mt-0.5 truncate">{sub}</div>}
          </div>
      </div>
  )
  ...
  return (
    <div className="min-h-screen bg-[#0B0F19] text-white font-sans selection:bg-emerald-500/30 overflow-x-hidden pb-20">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
         <div className="absolute top-[-20%] left-[-10%] w-[100vw] h-[100vw] md:w-[80vw] md:h-[80vw] bg-indigo-600/5 rounded-full blur-[100px] md:blur-[150px]"></div>
         <div className="absolute bottom-[-20%] right-[-10%] w-[80vw] h-[80vw] md:w-[60vw] md:h-[60vw] bg-emerald-600/5 rounded-full blur-[100px] md:blur-[150px]"></div>
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6">
        ...
        >
            ...
            <h1 className="text-4xl sm:text-5xl md:text-7xl font-black mb-6 tracking-tighter text-white leading-tight">
        ...
        >
            <div className="relative group">
                ...
                <div className="relative flex flex-col sm:flex-row items-stretch sm:items-center bg-[#131722] rounded-2xl p-2 border border-slate-800 shadow-2xl gap-2">
                    <div className="flex items-center flex-1 min-w-0">
                        <Search className="w-5 h-5 text-slate-500 ml-3 mr-2 flex-shrink-0" />
                        <input 
                            type="text" 
                            value={vin}
                            onChange={(e) => setVin(e.target.value.toUpperCase())}
                            placeholder="17 jegyű alvázszám..."
                            className="w-full bg-transparent text-white placeholder-slate-600 font-mono text-base md:text-lg py-3 md:py-4 focus:outline-none uppercase tracking-widest min-w-0"
                            maxLength={17}
                        />
                    </div>
                    <button 
                        type="submit" 
                        disabled={loading}
                        className="bg-white text-slate-950 font-bold px-6 py-3 md:py-4 rounded-xl hover:bg-emerald-400 transition-all flex items-center justify-center gap-2 shadow-lg disabled:opacity-50 flex-shrink-0"
                    >
                        {loading ? '...' : 'Keresés'}
                    </button>
                </div>
            ...
            <AnimatePresence>
                {error && (
                    <motion.div 
                        ...
                        className="mt-4 p-3 bg-red-500/10 border border-red-500/20 rounded-xl flex items-center justify-center gap-3 text-red-400 text-sm"
                    >
        ...
        <AnimatePresence>
            {result && (
                ...
                >
                    {/* --- CAR HEADER --- */}
                    <div className="relative rounded-[2rem] md:rounded-[3rem] overflow-hidden bg-gradient-to-b from-slate-900 to-[#0F131F] border border-slate-800 shadow-2xl">
                        ...
                        <div className="relative z-10 p-6 md:p-12 lg:p-16">
                            <div className="flex flex-col lg:flex-row gap-8 md:gap-12 items-start">
                                ...
                                <div className="w-full lg:w-5/12">
                                    <div className="relative aspect-[4/3] rounded-[1.5rem] md:rounded-[2rem] overflow-hidden bg-slate-800 border border-white/10 shadow-2xl group">
                                        {result.car.image_url ? (
                                        ...
                                        <div className="absolute top-3 left-3 md:top-4 md:left-4 bg-emerald-500/90 backdrop-blur-md text-white text-[9px] md:text-[10px] font-bold px-2 py-1 md:px-3 md:py-1.5 rounded-full uppercase tracking-wider flex items-center gap-1.5">
                                ...
                                <div className="flex-1 w-full">
                                    <div className="mb-8 md:mb-10">
                                        <div className="text-slate-400 font-mono text-xs md:text-sm tracking-widest mb-2 uppercase">
                                            {result.car.make}
                                        </div>
                                        <h2 className="text-4xl md:text-6xl lg:text-7xl font-black text-white tracking-tighter leading-[1.1] md:leading-[0.9] mb-6">
                                            {result.car.model}
                                        </h2>
                                        
                                        <div className="flex flex-wrap gap-2 md:gap-3">
                                            <div className="flex items-center gap-2 bg-slate-800/80 px-3 py-2 md:px-4 md:py-2.5 rounded-xl border border-slate-700/50">
                                                <span className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">VIN</span>
                                                <span className="font-mono text-emerald-400 text-xs md:text-sm font-bold tracking-wider">{result.car.vin}</span>
                                            </div>
                                            <div className="flex items-center gap-2 bg-slate-800/80 px-3 py-2 md:px-4 md:py-2.5 rounded-xl border border-slate-700/50">
                                                <span className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">Rendszám</span>
                                                <span className="font-mono text-white text-xs md:text-sm font-bold tracking-wider">{result.car.plate}</span>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div className="max-w-4xl mx-auto px-2">
                        <div className="flex items-center gap-4 md:gap-6 mb-12 md:mb-16 justify-center">
                            ...
                            <h3 className="text-2xl md:text-3xl font-black text-white flex items-center gap-3">
                                <History className="w-6 h-6 md:w-8 md:h-8 text-slate-600" /> 
                                Szerviztörténet
                            </h3>
                            <div className="h-px flex-1 max-w-[100px] bg-gradient-to-l from-transparent to-slate-800"></div>
                        </div>
                            ...
                            <div className="space-y-8 md:space-y-12">
                                                ...
                                                <div className="bg-[#131722] border border-slate-800 p-5 md:p-6 rounded-2xl hover:border-slate-700 transition-all shadow-xl group relative overflow-hidden">
                                                    ...
                                                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-4">
                                                        <div>
                                                            ...
                                                            <div className="text-lg md:text-xl font-bold text-white group-hover:text-emerald-400 transition-colors leading-tight">
                                                                {event.title}
                                                            </div>
                                                        </div>
                                                        <div className="self-start bg-slate-900 px-3 py-1 rounded-lg border border-slate-800 text-[10px] md:text-xs font-mono font-bold text-emerald-400 flex-shrink-0">
                                                    ...
                                                    <div className="space-y-3">
                                                        {event.description && (
                                                            <div className="flex gap-3 text-xs md:text-sm text-slate-300">
                                                                <Wrench className="w-3.5 h-3.5 text-slate-500 mt-0.5 flex-shrink-0" />
                                                                <p className="leading-relaxed">{event.description}</p>
                                                            </div>
                                                        )}
                                                        {event.notes && (
                                                            <div className="flex gap-3 text-xs md:text-sm text-slate-400 italic bg-slate-900/50 p-3 rounded-lg border border-slate-800/50">
                                ...
                                <div className="relative flex flex-col items-center pt-8">
                                     ...
                                     <div className="pl-10 md:pl-0 w-full md:w-auto text-left md:text-center">
                                        <div className="inline-block bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 px-5 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest">
