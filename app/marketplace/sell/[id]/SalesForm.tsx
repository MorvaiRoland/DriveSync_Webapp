...
import { 
    ...
    Loader2, Store, Check, Plus, Search, GripVertical 
} from 'lucide-react'
...
export default function SalesForm({ car }: { car: any }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        formData.append('location', location) // Ha támogatja az action
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-12">
            ...
            {/* 2. SZEKCIÓ: FELSZERELTSÉG (EXTRÁK) */}
                    ...
                    {Object.entries(CAR_FEATURES).map(([category, items]) => {
                        ...
                        return (
                            <div key={category} className="space-y-3">
                                <h4 className="text-xs font-bold uppercase text-slate-500 border-b border-slate-100 dark:border-slate-800 pb-2 mb-3">
                                    {category}
                                </h4>
                                    ...
                                    {filteredItems.map((item) => {
                                        ...
                                        return (
                                            <div 
                                                ...
                                                className={`cursor-pointer flex items-center gap-3 p-3 rounded-xl border transition-all duration-200 select-none group ${isSelected ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-700/50' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700'}`}
                                            >
                                                <div className={`w-5 h-5 rounded-md flex items-center justify-center border transition-colors ${isSelected ? 'bg-amber-500 border-amber-500 text-white' : 'bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-600 group-hover:border-slate-400'}`}>
                                                    {isSelected && <Check className="w-3.5 h-3.5" />}
                                                </div>
                                                <span className={`text-sm font-medium ${isSelected ? 'text-amber-900 dark:text-amber-100' : 'text-slate-600 dark:text-slate-400'}`}>
                                                    {item}
                                                </span>
                                            </div>
                                        )
            ...
            <div className="space-y-6 pt-8 border-t border-slate-100 dark:border-slate-800">
                ...
                <div className="space-y-3 pt-4">
                    ...
                    <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        ...
                        >
                            <div className="w-10 h-10 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mb-2 group-hover:bg-amber-100 dark:group-hover:bg-amber-900/30 transition-colors">
                                <Plus className="w-5 h-5 group-hover:text-amber-500" />
                            </div>
                ...
                <div className="bg-slate-50 dark:bg-slate-900/50 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 flex items-center justify-between cursor-pointer hover:border-amber-500/30 transition-colors" onClick={() => setIsPublic(!isPublic)}>
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-500">Mindenki számára látható lesz a hirdetés.</p>
                        </div>
                    </div>
