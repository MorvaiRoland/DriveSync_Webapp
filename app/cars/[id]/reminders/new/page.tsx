...
export async function generateMetadata(props: Props) {
  ...
  return {
    title: car ? `${car.make} ${car.model} | Új teendő` : 'Új teendő | DynamicSense',
    description: 'Szerviz emlékeztető tervezése.'
  }
}

export default async function NewReminderPage(props: Props) {
  const params = await props.params
  const supabase = await createClient()
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F7] dark:bg-[#000000] text-slate-900 dark:text-slate-100 font-sans transition-colors duration-700 relative overflow-x-hidden">
      ...
      >
        <ReminderFormClient car={car} serviceTypes={serviceTypes || []} />
      </main>
    </div>
  )
}