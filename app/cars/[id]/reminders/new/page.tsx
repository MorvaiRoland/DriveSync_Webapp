  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 pb-20 transition-colors duration-500 selection:bg-amber-500/30 selection:text-amber-600 relative overflow-x-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none">
          <div className="absolute top-[-10%] left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-indigo-500/10 dark:bg-indigo-500/5 rounded-full blur-[120px] animate-pulse-slow"></div>
      ...
      <div className="max-w-xl mx-auto px-4 -mt-8 relative z-20">
        <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-[2.5rem] shadow-2xl p-6 sm:p-10 border border-white/20 dark:border-slate-700 relative overflow-hidden group">
          ...
          {/* Dekoráció */}
          <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/5 rounded-full blur-3xl -mr-16 -mt-16 pointer-events-none group-hover:bg-indigo-500/10 transition-colors duration-700"></div>

          <form action={addReminder} className="space-y-8 relative z-10">
            ...
            <div className="bg-indigo-50/50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-800/50 rounded-2xl p-5 space-y-4 backdrop-blur-md">
               ...
               <label className="flex items-center gap-3 cursor-pointer p-3 hover:bg-white/50 dark:hover:bg-indigo-900/20 rounded-xl transition-all border border-transparent hover:border-indigo-200 dark:hover:border-indigo-800 group/check">
                  <div className="relative flex items-center">
                      ...
                      <CheckCircle2 className="pointer-events-none absolute h-3.5 w-3.5 left-1 top-1 text-white opacity-0 peer-checked:opacity-100 transition-opacity" />
                  </div>
                  <div>
            ...
            <div className="space-y-2 group">
                ...
                <div className="relative">
                    <textarea 
                      name="note" 
                      rows={3} 
                      className="block w-full rounded-2xl border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800/50 backdrop-blur-md p-4 text-slate-900 dark:text-white placeholder-slate-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all resize-none shadow-sm" 
            ...
            <div className="pt-6 flex gap-4 border-t border-slate-100 dark:border-slate-800">
              <Link 
                href={`/cars/${carId}`} 
                className="w-1/3 py-4 rounded-xl text-slate-500 dark:text-slate-400 font-bold text-center border border-transparent hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-sm uppercase tracking-wide flex items-center justify-center"
              >
                Mégse
              </Link>
              <button 
                type="submit" 
                className="relative w-2/3 py-4 rounded-2xl font-bold shadow-lg transition-all transform hover:-translate-y-1 active:scale-[0.98] bg-gradient-to-r from-indigo-500 to-violet-600 text-white hover:shadow-indigo-500/30 overflow-hidden group"
              >
...
function InputGroup({ label, name, type = "text", placeholder, required = false, icon }: any) {
    ...
    return (
      <div className="space-y-2 group">
        ...
        `}>
          {icon && (
            <div className={`pl-4 pr-2 transition-colors duration-300 ${focused ? 'text-indigo-500' : 'text-slate-400'}`}>
              {icon}
            </div>
          )}
