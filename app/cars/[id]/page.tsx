...
export default async function CarDetailsPage(props: { params: Promise<{ id: string }> }) {
  ...
  // --- STATISZTIKA ÉS KÖLTSÉGEK (JAVÍTVA) ---
  const totalCost = safeEvents.reduce((sum, event) => sum + (event.cost || 0), 0)
  
  // ITT VOLT A HIÁNY: Különválogatjuk a szerviz és üzemanyag költségeket
  ...
  // Intervallumok betöltése az autóból (vagy alapértelmezett)
  ...
  if (lastServiceEvent) {
     // Ha volt már szerviz, ahhoz viszonyítunk
     ...
     // Ha NEM volt szerviz, a kilométeróra állásból számolunk ciklust
  ...
  // Ha már közel járunk (kevesebb mint 2000 km VAGY 30 nap van hátra)
               ...
               <div className="p-6 space-y-6">
                 ...
                 {lastServiceEvent ? (
                     <div>
                       ...
                       <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden">
                          <div className={`h-full rounded-full transition-all duration-1000 ${daysRemaining <= 0 ? 'bg-red-500' : daysRemaining <= 30 ? 'bg-amber-500' : 'bg-gradient-to-r from-blue-400 to-blue-600'}`} style={{ width: `${Math.min((daysSinceService / serviceIntervalDays) * 100, 100)}%` }}></div>
                       </div>
                     </div>
                 ) : (
