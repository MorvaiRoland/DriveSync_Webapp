...
export default async function Home() {
            ...
            <div className="flex h-16 justify-between items-center">
              ...
              <div className="flex items-center gap-4">
                <div className="hidden sm:flex flex-col items-end">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Bejelentkezve</span>
  ...
  return (
    <div className="min-h-screen w-full bg-slate-950 font-sans text-slate-200 flex flex-col lg:flex-row selection:bg-amber-500/30 overflow-x-hidden">
      ...
      {/* --- BAL OLDAL: BEMUTATKOZÁS (SCROLLABLE) --- */}
...
function CarCard({ id, make, model, plate, status, year, mileage, fuel }: any) {
  return (
    <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group cursor-pointer">
      <Link href={`/cars/${id}`} className="block h-full">
        <div className="h-40 bg-slate-100 relative flex items-center justify-center overflow-hidden">
           ...
           <svg className="w-20 h-20 text-slate-300 group-hover:text-slate-400 transition-colors relative z-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
           ...
           <div className={`absolute top-4 right-4 px-3 py-1 text-[10px] font-black rounded-full uppercase tracking-wider shadow-sm ${status === 'OK' ? 'bg-white text-emerald-600' : 'bg-amber-100 text-amber-700'}`}>
